import type { HealthResponse, RunsResponse, TaskWithStatus } from "@breckr/shared";
import {
  fetchTasks,
  createTask,
  updateTask,
...
export async function loadSnapshot(
  ...
  const [tasksResponse, runs, health] = await Promise.all([
    fetchTasks(),
    fetchRuns(runFilters),
    fetchHealth(),
  ]);

  return { tasks: tasksResponse.tasks, runs, health };
}
